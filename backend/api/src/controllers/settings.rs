...
fn get(session: Protected, db: DigesterDbConn) -> JsonResponse {
    ...
    match db::users_find_by_id(&db.0, user_id.into()) {
        Ok(user) => {
...
fn update(
    ...
    let user = match db::users_find_by_id(&db.0, user_id.into()) {
        Ok(user) => user,
        Err(err) => {
    ...
    match db::users_update_timezone(&db.0, user_id.into(), new_tz) {
        Ok(()) => {
